...
p = parcellation(fullfile('/imaging/henson/users/dv01/Toolboxes/osl/parcellations/fMRI_parcellation_ds8mm.nii.gz'));

%mni_coords  = p.template_coordinates;
mni_coords = osl_mnimask2mnicoords(fullfile('/imaging/henson/users/dv01/Toolboxes/osl/std_masks/MNI152_T1_8mm_brain.nii.gz'));
    ...
    if exist(T1file,'file')
        ...
        for m = 1:length(modalities)
            ...
            if length(modalities{m})>1
                ...
                SS.dirname = [infile '_SB_298' strcat(modalities{m}{:}) '_BFn'];
                S.outfile = [infile '_SB_298' strcat(modalities{m}{:})];
            else
                SS.fuse = 'no';
            ...
            DS = ROInets.get_node_tcs(DS,p.parcelflag,'spatialBasis');
...
signal_raw = cell(1,numel(freqbands));
...
for ii = 1:length(freqbands)
    ...
    T1file = fullfile('/imaging/henson/users/dv01/defacing_project',anat{ana},sprintf('sub-Sub%04d_ses-meg1_T1w.nii',sub));
        ...
        if exist(T1file,'file')
            ...
            infile = fullfile(processed_pth,(sprintf('sub-Sub%04d',sub)),'_ffdspmeeg_PCA_298MEGMEGPLANAR');
            D = spm_eeg_load(infile);
...
signal_raw = signal_raw(~cellfun('isempty',signal_raw));

% save pe_cov_sb covariance power_env

%% HMM inference (Power estimation)

% Preparing T and data after smoothing
T = cell(307,1);
data_new = cell(307,1);

for sub=1:307
    
        Hen_lc_sep = signal_raw{sub};
        ...
        data_new{sub} = Hen_lc_sep';
        T{sub} = size(Hen_lc_sep,2);
...
options_signflip = struct();
options_signflip.maxlag = 5;
options_signflip.verbose = 1;
options_signflip.nbatch = 3;
options_signflip.noruns = 5;
options_signflip.partial = 1;
options_signflip.maxcyc = 500;

flips = findflip(data,T,options_signflip);
...
options.pca = 38*4;          % The PCA dimensionality reduction is 4 times the number of ROIs
...
[bLcomI,bLcomL,acc1,acc2] = mkl_ens_nestedcv(V,labels,'Nfold_o',5,'Nfold_i',10,'Nrun',10);
